import type { ParentUserType } from './scoring'
...
const primaryPartA: Question[] = [
  {
    ...
    dimension: 'context',
    options: [
      { label: '已经觉得不太对劲，但不确定问题出在哪里、该怎么办',         value: 'A', score: 3, userType: 'A' },
      { label: '在主动了解各种AI教育的方法，想找一个真正有用的',           value: 'B', score: 4, userType: 'B' },
      { label: '没怎么想过这件事，孩子学习没出问题就行',                   value: 'C', score: 1, userType: 'C' },
      { label: '知道AI很重要，提醒过孩子别太依赖，但没有具体做什么',       value: 'D', score: 2, userType: 'A' },
    ],
  },
...
const primaryPartB: Question[] = [
...

  {
    ...
    options: [
      ...
      { label: '再问AI一次，如果它还是这么说就信它吧',               value: 'B', score: 3 },
      { label: '觉得差不多就行了，不想那么多',                       value: 'C', score: 1 },
      { label: '觉得可能是我记错了，用AI的答案比较保险',             value: 'D', score: 2 },
    ],
  },
...
const middlePartA: Question[] = [
  {
    ...
    dimension: 'context',
    options: [
      { label: '我们商量过规则——什么情况用、怎么用、我怎么检查',     value: 'A', score: 4, userType: 'B' },
      { label: '知道孩子在用，提醒过别太依赖，但没有具体的约定',     value: 'B', score: 2, userType: 'A' },
      { label: '不太了解孩子怎么用的，也不确定该不该管',             value: 'C', score: 1, userType: 'C' },
      { label: '时不时会问一下，但孩子大了不太愿意细说',             value: 'D', score: 3, userType: 'A' },
    ],
  },
...
const seniorPartA: Question[] = [
  {
    ...
    dimension: 'context',
    options: [
      { label: 'AI必须会用，但前提是孩子有独立思考的基础，两者不能偏废',   value: 'A', score: 4, userType: 'B' },
      { label: '既担心不会用未来竞争力不够，又怕用太多丢了独立思考——很矛盾', value: 'B', score: 3, userType: 'A' },
      { label: '高中课业压力大，用AI能提效就用，先把成绩搞好再说',           value: 'C', score: 2, userType: 'C' },
      { label: '孩子大了有自己的判断，我不太操心这件事',                     value: 'D', score: 1, userType: 'C' },
    ],
  },
  ...
  {
    id: 'SA5',
    text: '孩子使用AI时，您观察到他对AI给出的内容通常持什么态度？',
    dimension: 'active_judge',
    mirror: 'SB2',
    options: [
      { label: '会主动核查，遇到说不准的内容会去验证，不是拿到就直接用', value: 'A', score: 4 },
      { label: '大体接受，但偶尔会对某些内容说「这个对吗」',             value: 'B', score: 3 },
      { label: '基本直接用，没有观察到他主动质疑什么',                   value: 'C', score: 2 },
      { label: '完全信任，从来不质疑AI给的内容',                         value: 'D', score: 1 },
    ],
  },
...
const seniorPartB: Question[] = [
...

  {
    ...
    options: [
      ...
      { label: '再问AI一次，让它综合各方给我一个结论',                   value: 'B', score: 3 },
      { label: '选看起来最可信的那个用就行',                             value: 'C', score: 2 },
