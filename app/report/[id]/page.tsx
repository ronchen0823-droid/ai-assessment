...
const LEVEL_CONFIG: Record<string, { color: string; bg: string }> = {
  '良好': { color: '#059669', bg: '#f0fdf4' },
  '中等': { color: '#d97706', bg: '#fffbeb' },
  '待提升': { color: '#6366f1', bg: '#eef2ff' },
}
    ...
    <div style={{ background: '#f7f8fc', minHeight: '100vh', fontFamily: "'Noto Sans SC', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .rcard { background:#fff; border-radius:20px; padding:24px; margin-bottom:12px; border:1px solid #f1f5f9; animation:fadeUp 0.4s ease both; }
        .dim-row { display:flex; align-items:flex-start; padding:12px 0; border-bottom:1px solid #f8fafc; }
        .dim-row:last-child { border-bottom:none; padding-bottom:0; }
        .fcard { background:#f8f9fc; border-radius:14px; padding:16px; margin-bottom:10px; }
        .fcard:last-child { margin-bottom:0; }
      `}</style>

      <div style={{ position:'sticky', top:0, zIndex:10, background:'rgba(247,248,252,0.92)', backdropFilter:'blur(10px)', borderBottom:'1px solid #f1f5f9', padding:'12px 20px' }}>
      ...
      <div style={{ maxWidth:480, margin:'0 auto', padding:'16px 16px 48px' }}>
        ...
        {surveyData && (
          <div className="rcard" style={{ animationDelay:'0.05s' }}>
        ...
        {generating && (
          <div className="rcard" style={{ textAlign:'center', padding:'40px 24px' }}>
        ...
        {report && (
          <>
            <div className="rcard" style={{ animationDelay:'0.1s', background:'#1e293b' }}>
              <div style={{ fontSize:10, color:'#64748b', marginBottom:10, letterSpacing:'0.12em', textTransform:'uppercase' }}>核心发现</div>
              <div style={{ fontSize:21, fontWeight:700, color:'#f8fafc', lineHeight:1.4, fontFamily:"'Noto Serif SC', serif" }}>{report.headline}</div>
            </div>

            <div className="rcard" style={{ animationDelay:'0.15s' }}>
              <div style={{ fontSize:12, fontWeight:600, color:'#94a3b8', marginBottom:14, letterSpacing:'0.08em' }}>三个维度</div>
                  ...
                  <div key={i} className="dim-row">
                    <div style={{ flex:1 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:5 }}>
                        <span style={{ fontSize:15, fontWeight:600, color:'#1e293b' }}>{dim.name}</span>
                        <span style={{ fontSize:11, fontWeight:500, color:cfg.color, background:cfg.bg, padding:'2px 9px', borderRadius:20 }}>{dim.level}</span>
                      </div>
                      <div style={{ fontSize:13, color:'#64748b', lineHeight:1.6, fontWeight:300 }}>{dim.desc}</div>
                    </div>
                  </div>
                )
              })}
            ...
            <div className="rcard" style={{ animationDelay:'0.2s' }}>
              <div style={{ fontSize:12, fontWeight:600, color:'#94a3b8', marginBottom:14, letterSpacing:'0.08em' }}>关键发现</div>
              {report.findings?.map((f, i) => (
                <div key={i} className="fcard">
                  <div style={{ fontSize:15, fontWeight:600, color:'#1e293b', marginBottom:7, lineHeight:1.4 }}>{f.title}</div>
                  <div style={{ fontSize:13, color:'#64748b', lineHeight:1.75, fontWeight:300 }}>{f.body}</div>
                </div>
              ))}
            </div>

            <div className="rcard" style={{ animationDelay:'0.25s', background:'linear-gradient(135deg,#eef2ff 0%,#f5f3ff 100%)', border:'1px solid #e0e7ff' }}>
              <div style={{ fontSize:10, fontWeight:600, color:'#6366f1', letterSpacing:'0.12em', marginBottom:10, textTransform:'uppercase' }}>今天就能做的一件事</div>
              <div style={{ fontSize:15, color:'#3730a3', lineHeight:1.75, fontWeight:500 }}>{report.action}</div>
            </div>
