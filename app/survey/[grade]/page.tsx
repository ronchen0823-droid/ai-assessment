...
export default function SurveyPage() {
  ...
  const [phase, setPhase] = useState<'intro-a' | 'part-a' | 'open-a' | 'intro-b' | 'part-b' | 'open-b' | 'submitting'>('intro-a')
  ...
  // 开放题：Part A
  const openQ = isPartA ? questions.openA : questions.openB
  if (phase === 'open-a') {
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-12">
        <div className="max-w-md w-full">
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
            <div className="text-center mb-6">
              ...
              <div className="inline-block bg-indigo-100 text-indigo-700 text-xs font-medium px-3 py-1 rounded-full mb-3">
                家长问卷已完成 ✓
              </div>
              <h2 className="text-xl font-bold text-slate-900 mb-2">再说两句（选填）</h2>
            </div>

            <div className="mb-6">
              <p className="text-sm text-slate-700 leading-relaxed mb-4">{questions.openA.text}</p>
              <textarea
                ...
                placeholder="请输入您的回答..."
                value={parentOpen}
                onChange={(e) => setParentOpen(e.target.value)}
              />
            </div>

            <button
              onClick={() => { setPhase('intro-b'); setCurrentQuestion(0) }}
              className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3.5 rounded-2xl transition-colors"
            >
              {parentOpen ? '继续 →' : '跳过，继续 →'}
            </button>
      ...
      <div className="min-h-screen flex items-center justify-center px-4 py-12">
        <div className="max-w-md w-full">
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
            ...
            <div className="mb-6">
              <p className="text-sm text-slate-700 leading-relaxed mb-4">{questions.openB.text}</p>
              <textarea
                className="w-full p-4 border border-slate-200 rounded-2xl text-sm resize-none focus:outline-none focus:ring-2 focus:ring-emerald-500"
                rows={4}
                placeholder="请输入你的回答..."
                value={studentOpen}
                onChange={(e) => setStudentOpen(e.target.value)}
              />
            </div>
